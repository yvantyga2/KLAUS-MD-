...
const mistralSystemPrompt = "You are 𝐊𝐋𝐀𝐔𝐒-𝐌𝐃, an intelligent bot. Someone mentioned your owner. Respond with helpful and friendly assistance.";
...
const mistral = async (m, Matrix) => {
    ...
    if (isBotOrOwnerMentioned) {
        ...
        if (text === `@${botNumber}` || text === `@${ownerNumber}`) {
            await Matrix.sendMessage(m.from, { audio: { url: 'https://i.imgur.com/vLvmKB3.mp4' } }, { quoted: m });
            return;
        }
